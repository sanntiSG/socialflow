...
router.get('/:network/auth', requireAuth, (req: Request, res: Response) => {
  ...
  const oauthUrls: Record<string, string> = {
    facebook: `https://www.facebook.com/v18.0/dialog/oauth?client_id=${process.env.FACEBOOK_APP_ID}&redirect_uri=${process.env.BACKEND_URL}/api/auth/facebook/callback&scope=public_profile,email,pages_show_list,pages_manage_posts,pages_read_engagement&state=${req.query.token}`,
    ...
    instagram: `https://www.facebook.com/v18.0/dialog/oauth?client_id=${process.env.FACEBOOK_APP_ID}&redirect_uri=${process.env.BACKEND_URL}/api/auth/instagram/callback&scope=public_profile,email,instagram_basic,instagram_content_publish,pages_show_list,pages_read_engagement&state=${req.query.token}`,
  };
